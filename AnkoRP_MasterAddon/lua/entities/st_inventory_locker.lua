...
if SERVER then
    ...
    net.Receive( "ST_InventoryTransaction", function( _, ply )
        ...
        if typ == "store" then
            ...
            return end
            if ply:GetWeaponTable()[ GetFinalParent( ply:GetActiveWeapon():GetClass() ) ] == ply:GetActiveWeapon():GetClass() then
                DarkRP.notify( ply, 1, 4, "That weapon is already in your inventory." )
            return end
            if ply:GetWeaponTable()[ GetFinalParent( ply:GetActiveWeapon():GetClass() ) ] and ply:GetWeaponTable()[ GetFinalParent( ply:GetActiveWeapon():GetClass() ) ] != "NULL" then
                DarkRP.notify( ply, 1, 8, "Could not store your " .. weapons.Get( ply:GetActiveWeapon():GetClass() ).PrintName .. " as you already have a " .. weapons.Get( ply:GetWeaponTable()[ GetFinalParent( ply:GetActiveWeapon():GetClass() ) ] ).PrintName .. ". Scrap it to store your new weapon." )
            else
                ply:GetWeaponTable()[ GetFinalParent( ply:GetActiveWeapon():GetClass() ) ] = ply:GetActiveWeapon():GetClass()
                ply:AddWeaponToTable( ply:GetActiveWeapon():GetClass() )
